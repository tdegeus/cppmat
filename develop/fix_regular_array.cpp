...
SECTION( "where" )
{
  ...
  std::vector<size_t> g = A.greater   (.5).where();
  std::vector<size_t> l = A.less_equal(.5).where();

  REQUIRE( g.size() + l.size() == A.size() );

  for ( auto &i : g ) REQUIRE( A[i] >  .5 );
  for ( auto &i : l ) REQUIRE( A[i] <= .5 );
}
