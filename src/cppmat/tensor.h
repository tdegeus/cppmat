...
private:
  ...
  size_t         m_nd=0;    // number of dimensions
  size_t         m_size=0;  // size of the data array
...
public:
  ...
  tensor2s(size_t nd, const X *D, bool dense)
  {
    // - change size of internal storage
    resize(nd);
  ...
  // copy from Eigen array
  #ifdef CPPMAT_EIGEN
  tensor2s(const Eigen::Matrix<X,Eigen::Dynamic,Eigen::Dynamic,Eigen::RowMajor> &D)
  {
    // - check size
    assert( D.rows() == D.cols() );
    // - change size of internal storage
    ...
    for ( size_t i = 0 ; i < m_nd ; ++i )
      for ( size_t j = i ; j < m_nd ; ++j )
        m_data[i*m_nd-(i-1)*i/2+j-i] = D(i,j);
  }
  #endif
  ...
  operator tensor2<U> () const
  {
    ...
    for ( size_t i = 0 ; i < m_nd ; ++i )
      for ( size_t j = i ; j < m_nd ; ++j )
        out[i*m_nd+j] = out[j*m_nd+i] = static_cast<U>(m_data[i*m_nd-(i-1)*i/2+j-i]);
...
public:
  ...
  // allocate tensor, copy from raw pointer "D"
  tensor2d(size_t nd, const X *D, bool dense)
  {
    // - change size of internal storage
    resize(nd);
  ...
  // copy from Eigen array
  #ifdef CPPMAT_EIGEN
  tensor2d(const Eigen::Matrix<X,Eigen::Dynamic,Eigen::Dynamic,Eigen::RowMajor> &D)
  {
    // - check size
    assert( D.rows() == D.cols() );
    // - change size of internal storage
...
template<class X> inline void tensor4<X>::printf(std::string fmt) const
{
...
template<class X> inline void tensor2s<X>::printf(std::string fmt) const
{
  size_t nd = m_nd;
  size_t i,j;
...
template<class X> inline void tensor2d<X>::printf(std::string fmt) const
{
  size_t nd = m_nd;
  size_t i,j;
...
template<class X> inline void vector<X>::printf(std::string fmt) const
{
  size_t nd = m_nd;
...
inline tensor4<double> identity4(size_t nd)
{
...
inline tensor4<double> identity4rt(size_t nd)
{
  tensor4<double> I(nd,0.0);
...
inline tensor4<double> identity4II(size_t nd)
{
  tensor4<double> I(nd,0.0);
...
inline tensor4<double> identity4s(size_t nd)
{ return (identity4(nd)+identity4rt(nd))/2.; }

// -------------------------------------------------------------------------------------------------

inline tensor4<double> identity4d(size_t nd)
{ return identity4s(nd)-identity4II(nd)/static_cast<double>(nd); }

// -------------------------------------------------------------------------------------------------

inline tensor2d<double> identity2(size_t nd)
{
